...
function dYdt = twoIon_wPhoto_wVmeffects_ode(T, Y, p, pS, myC)
...
ThT_cphoto = Y(3)/cellV;   %cytoplasm photosensitised ThT conc in [umoles/um3]
ThT_mphoto = Y(4)/mitV;    %mitochondrial photosensitised ThT conc in [umoles/um3]
...
dYdt(3) = Kon_c*(Y(1)^N/(K2^N+(Y(1)^N)))-Koff_c*Y(3); % ThT_cphoto
dYdt(4) = Kon_m*(Y(2)^N/(K^N+(Y(2)^N))) - Koff_m*Y(4); % ThT_mphoto
